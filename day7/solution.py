  ...
  with open("input.txt", "r", encoding="utf-8") as file:
    ...
    for row in diagram[2:]:
      ...
      for col in range(0, cols):
        if diagram[current_row_index - 1][col] == "|":
          if diagram[current_row_index][col] == "^":
            tachyon_beam_split_times += 1
            # beam split to the left side of the spliter
            if col > 0:
              diagram[current_row_index] = replace_char_in_str(diagram[current_row_index], "|", col - 1)
            # beam split to the right side of the spliter
            if col < cols - 1:
              diagram[current_row_index] = replace_char_in_str(diagram[current_row_index], "|", col + 1)
          # beam go downward
          else:
            diagram[current_row_index] = replace_char_in_str(diagram[current_row_index], "|", col)
...
def get_beam_downward_count(diagram, current_row, current_col, memo=None):
  ...
  if current_row == len(diagram) - 1:
    return 1
  else:
    current = diagram[current_row][current_col]
    if current == "^":
      res = get_beam_downward_count(diagram, current_row, current_col - 1, memo) + get_beam_downward_count(
        diagram, current_row, current_col + 1, memo
      )
    else:
